 ...
 * @author yeko
 */
public class UnfollowTagServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            FollowTagDAO unfollowTag = new FollowTagDAO();
            int result = unfollowTag.unfollowTag(tagID, unfollowerID);
            if (result == 0) {
                request.setAttribute("unfollowTagMessage", "unfollowed...");
                //RequestDispatcher dispatcher = request.getRequestDispatcher(returnPage);
                //dispatcher.forward(request, response);
                response.sendRedirect(returnPage);
                return;
            } else {
                request.setAttribute("unfollowTagMessage", "ooppss!");
                //RequestDispatcher dispatcher = request.getRequestDispatcher(returnPage);
                //dispatcher.forward(request, response);
                response.sendRedirect(returnPage);
                return;
